...
class Game:
  ...
  def step(self):
    ...
    if action_size == 0 or "w__K" not in self.team[0].players or "b__K" not in self.team[1].players:
      ...
      state_action = str(cycle) + "\t" + str(turn) + "\t" + str(self.move_count) + "\t" + str(state).replace(" ","") + "\t" +  str(value) + "\t" + str(action_sparse) + "\t" + action_verbose + "\t" + str(action_size) +"\t" + str(0) + "\n"
      ...
      #print("SELF.MOVE_COUNT:\t{}".format(self.move_count))
      #print("TURN:\t{}\tTYPE(TURN):\t{}\tINT(TURN):\t{}".format(turn,type(turn),int(turn)))   
...
class userInput:
  ...
  def get_num_trials(self):
    ...
    while not done:
      try:
        trials = int(
          input("How many trials would you like to run? [1 - 20,000,000] "))
        if trials > 20000000 or trials < 0 or not isinstance(trials, int):
          raise Incorrect_Input_error
      except Incorrect_Input_error:
        print("Please select a choice within the proposed range")
        print("[1 - 20,000,000]")
...
def run_trials(user_input,num_jobs):
  ...
  results     = [pool.apply_async(run_trial, args=(user_input,)) for x in range(num_cycles)] 
...
def main():
  ...
  for i in range(num_jobs):
    ...
    run_trials(user_input,num_jobs)
